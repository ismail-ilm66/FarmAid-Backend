...
exports.generateOtp = async (email) => {
    ...
    if (existingOtp?.otpDisabledTime && currentTime < existingOtp.otpDisabledTime) {
      const remainingTime = Math.ceil((existingOtp.otpDisabledTime - currentTime) / 1000);
      return {code: 409, status: false, message: `OTP generation is disabled. Try again in ${remainingTime} seconds.`};
    }
    ...
    return {code: 201, status: true, message: "OTP Generated and Sent Successfully", otp: otpCode};
};
...
exports.verifyOtp = async (email, submittedOtp) => {
    ...
    if (!otpRecord) return {code: 404, status: false, message: "No OTP record found for this email"};
    
    const currentTime = new Date();

    if (otpRecord.otpExpiry < currentTime) return {code: 401, status: false, message: "OTP has expired"};
    ...
    if (retriesLeft <= 0) {
        ...
        await otp.update({ otpRetries: 0, otpDisabledTime }, { where: { email } });

        return {code: 401, status: false, message: "Invalid OTP. No retries left. OTP generation disabled for 2 minutes."};
    } else {
        await otp.update({ otpRetries: retriesLeft }, { where: { email } });
        return {code: 401, status: false, message: `Invalid OTP. You have ${retriesLeft} retries left.`};
    }
    }
