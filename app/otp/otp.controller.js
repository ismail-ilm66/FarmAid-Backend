...
exports.sendOtp = async (req, res) => {
    ...
    if(!email) return res.status(400).json({code: 400, status: false, message: "email is required"});
...
exports.verifyOtp = async (req, res) => {
    ...
    if(!email) return res.status(400).json({code: 400, status: false, message: "email is required"})
    if(!otp) return res.status(400).json({code: 400, status: false, message: "otp is required"})
